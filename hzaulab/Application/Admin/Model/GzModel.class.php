<?php
	...
	class GzModel extends RelationModel {
		// 获取所有的盆栽信息
		public function getAllGz($value='') {
			# code...
			// $show_date 表示当前的时间
			$show_date=date('Y-m-d');
			$modePz = M("Gz");
			$deviceLists = $modePz -> order('id desc') -> where("$value = name") -> select();
		...
		public function modifyGz($value='', $content)
		{
			$model = M("Gz");
